...
> ⚠️ **Installed the very first build (v0.4.0)?** It shipped under the old name **“MiniMax Agent”**. The app was renamed to **MiniMax Studio**, and because the name changed, that early build **won't auto-update**. If you have **MiniMax Agent** installed, please **uninstall it** the way you normally remove an app on your system (Windows: *Settings → Apps*; macOS: drag it from *Applications* to the Trash; Linux: remove the `.deb`/AppImage you installed) and grab the latest **MiniMax Studio** from the **[Releases page](https://github.com/eduardoabreu81/minimax-agent-gui/releases/latest)**. You only need to do this once — every version after that updates itself.
